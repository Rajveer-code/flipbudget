...
## Extension: designing for multiple plausible scorers, not one canonical scorer

The scorer-mismatch finding (`SCORER_MISMATCH_MAJOR_FINDING.md`) means the audit-design problem stated at the top of this document is incomplete in one respect: it optimizes allocation for estimating *one* scorer's (α, β), implicitly assuming there is one canonical scorer to audit. There are at least two (`exact_match`, `score_boxed`), and nothing rules out more for other benchmarks. The generalized problem: given budget `B`, choose an allocation across strata **and across which scorer's verdict to stratify on** that jointly minimizes uncertainty in the quantity that actually matters for the estimand — the T-C expansion's joint design (`TC_EXPANSION_PREREGISTRATION.md`) is a first, concrete instance of this: it stratifies on both scorers simultaneously plus an explicit disagreement stratum, rather than assuming one scorer's stratification suffices. This is now the audit-design contribution's stated scope, not an afterthought: **a design that only supports one canonical scorer will silently misallocate budget whenever a benchmark has more than one plausible automated criterion**, which this session's evidence suggests is not a rare edge case (MATH-Hard alone has two in active use).
